  ...
  if (client.connect(server, serverPort)) {
    ...
    while (client.connected() && millis() - start < 2000) {
      if (client.available()) {
        ...
        //  начало цифровых значений
        int dataStart = 0;
        while (dataStart < response.length() && 
              !isdigit(response.charAt(dataStart)) {
          dataStart++;
        }
      
        if (dataStart < response.length()) {
          String cleanResponse = response.substring(dataStart);
          
          int firstComma = cleanResponse.indexOf(',');
          int secondComma = cleanResponse.indexOf(',', firstComma + 1);
